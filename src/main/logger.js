 ...
 *  - Startup rotation: on the FIRST log() call of each app run, upv.log is
 *    always rotated into upv.log.1 (regardless of file size).
 *      upv.log.2 → upv.log.3  (shift)
 *      upv.log.1 → upv.log.2  (shift)
 *      upv.log   → upv.log.1  (shift)
 *      create fresh upv.log
 *  - Size-based rotation (MAX_LOG_BYTES = 5 MB) is kept as a safety net for
 *    very long-running sessions (e.g. kiosk mode without restarts).
...
const MAX_LOG_BYTES = 5 * 1024 * 1024; // 5 MB (safety-net for very long sessions)
...
/**
 * Performs the archive-shift (highest first to avoid overwrite):
 *   upv.log.2 → upv.log.3
 *   upv.log.1 → upv.log.2
 *   upv.log   → upv.log.1
 *
 ...
 */
function _shiftArchives(logPath, fs) {
...
/**
 * Performs startup rotation – rotates the current log file on the first
 * log() call of each application run, so every startup begins with a
 * fresh upv.log.
 *
 * If upv.log does not exist yet, rotation is skipped silently.
 *
 * @param {string} logPath  – absolute path to upv.log
 * @param {object} fs       – fs module (real or mocked)
 */
function rotateOnStartup(logPath, fs) {
  try {
    try {
      fs.statSync(logPath);
    } catch {
...
/**
 * Performs size-based log rotation (safety net for very long sessions).
 *
...
function createRotatingLogger(deps = {}) {
  ...
  /**
   * Writes a log entry. On the first call of this app run, the existing
   * log file is rotated so each startup has its own fresh log file.
   * @param {string} source   – 'app' | 'window'
   * @param {string} message
   * @param {Date}   [now]
   */
  function log(source, message, now) {
    try {
      ...
      // Startup rotation – once per process lifetime
      if (!_startupRotated) {
        _startupRotated = true;
        rotateOnStartup(p, fs);
      }

      // Size-based rotation as safety net for very long-running sessions
      rotateLog(p, fs);
